...
if [[ ! -f "$ENV_FILE" ]]; then
  echo "Missing $ENV_FILE — copy .env.prod.example and fill required values." >&2
  exit 1
fi
...
PUBLIC_WEB_URL="${PUBLIC_WEB_URL:-https://www.kuberaiq.com}"
PUBLIC_API_URL="${PUBLIC_API_URL:-https://api.kuberaiq.com}"
...
if [[ -z "$JWT_SECRET" || "$JWT_SECRET" == "dev-only-change-me" ]]; then
  echo "JWT_SECRET is required for production (generate with: openssl rand -base64 48 | tr -dc 'A-Za-z0-9' | head -c 48)" >&2
  exit 1
fi
...
# Auto-disable mocks when credentials are present (explicit .env.prod values take precedence)
...
# Use custom domains only when DNS resolves; otherwise Azure hostnames
...
echo "==> Deploying KuberAIQ (Track A production)"
...
# Use JSON file — az CLI splits values on semicolons (connection strings) and mangles JSON arrays (CORS).
...
echo "==> Running migrations (including 008_subscriptions)"
...
# Prefer custom API URL for smoke tests when live
...
echo "  Public web: ${PUBLIC_WEB_URL}  (after Hostinger DNS)"
echo "  Public API: ${PUBLIC_API_URL}  (after Hostinger DNS)"
echo ""
echo "Next steps:"
echo "  1. ./scripts/check-kuberaiq-dns.sh"
echo "  2. ./scripts/setup-kuberaiq-domains.sh"
echo "  3. ./scripts/verify-track-a.sh"
